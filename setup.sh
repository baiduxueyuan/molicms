...
# get default prefix
if test -f $HOME/.mc_installed ; then
  def_prefix=`cat $HOME/.mc_installed`
elif test "$HOME" = "/" || test "$HOME" = "/root" ; then
  def_prefix=/usr/local/moilicms
else
  def_prefix=$HOME/moilicms
fi
...
do_clean()
{
  echo -n "Cleaning ... "
  rm -rf setup.log
  rm -rf xapian-core-scws-* scws-* libevent-* moilicms-*
  echo "done"
}
...
while [ $i -lt $# ] ; do
  ...
  case $opt in
	...
	"--enable-debug"|"--enable-memory-cache")
	  mc_add_option="$mc_add_option $arg"
	;;
	"--jobs")
...
echo
echo "+==========================================+"
echo "| Welcome to setup moilicms(full)         |"
echo "| 欢迎使用 moilicms (完整版) 安装程序     |"
echo "+------------------------------------------+"
...
# install/upgrade moilicms
new_file=`ls ./packages/moilicms-*.tar.bz2`
new_version=`echo $new_file | sed 's#^.*moilicms-\(.*\)\.tar\.bz2#\1#'`
if test -z "$new_version" ; then
  echo "ERROR: Missing moilicms package (缺少 moilicms 安装包)"
  exit 2
fi
echo "Extracting moilicms package ($new_version) ..."
tar -xjf $new_file
cd moilicms-$new_version
echo "Configuring moilicms ..."
./configure --prefix=$prefix --with-scws=$prefix $mc_add_option \
--with-libevent=$prefix --with-xapian=$prefix >> ../setup.log 2>&1
if test $? -ne 0 ; then
  setup_abort "configure moilicms"
fi
echo "Compiling & installing moilicms ..."
make clean >> ../setup.log 2>&1
make $mk_add_option install >> ../setup.log 2>&1
if test $? -ne 0 ; then
  setup_abort "compile moilicms"
fi
cd ..
...
echo "| 安装成功，感谢选择和使用 moilicms              |"
echo "+-------------------------------------------------+"
echo "| 说明和注意事项：                                |"
echo "| 1. 开启/重新开启 moilicms 服务程序，命令如下： |"
echo "|    $prefix/bin/ctl.sh restart"
echo "|    强烈建议将此命令写入服务器开机脚本中 如:/etc/rc.local  |"
echo "|                                                 |"
echo "| 2. 所有的数据库数据将被保存在下面这个目录中：     |"
echo "|    $prefix/data"
